...
import time  # Für Logging der Trial-Dauer


##############################################
# Funktion zum Setzen des Zufalls-Seeds
##############################################
...
##############################################
# 4) TRAINING + EARLY STOPPING (Objective Funktion)
##############################################

# def objective(trial):
#     start_time = time.time()  # Startzeit für Logging

#     # Vorschlagen von Hyperparametern mit erweiterten Bereichen
#     hidden_size = trial.suggest_int('hidden_size', 32, 512)
#     num_layers = trial.suggest_int('num_layers', 1, 5)
#     dropout = trial.suggest_float('dropout', 0.1, 0.7, step=0.05)  # Feinere Schritte
#     learning_rate = trial.suggest_float('learning_rate', 1e-6, 1e-2, log=True)  # Größerer Bereich
...
#                 batch_first=True,
#                 dropout=dropout if num_layers > 1 else 0  # Dropout nur zwischen den LSTM-Schichten
#             )
...
if __name__ == "__main__":
    # Setzen des Zufalls-Seeds für Reproduzierbarkeit
    set_seed(42)

    file_path = "Daten 2020 - 2024.xlsx"  # Aktualisiert
    ...
    # Überprüfe, ob Daten für 2024 vorhanden sind
    ...
    # Wenn Daten für 2024 vorhanden sind:
    ...
    # Speichere die entsprechenden Zeitstempel für die Testziele
    if not df_test.empty:
    ...
    # 5) Skalierung: Fit nur auf Trainingsdaten
    ...
    # )
    # study.optimize(objective, n_trials=100, timeout=7200)  # Erhöhte n_trials und timeout
    ...
    final_optimizer = torch.optim.Adam(
        ...
        weight_decay=1e-5  # Optional: L2-Regularisierung
    )
    ...
    # Definiere eine neue Trainingsfunktion oder passe die vorhandene an, um das finale Modell zu trainieren
    EPOCHS = 100
    PATIENCE = 10
    best_val_loss = float('inf')
    early_stop_counter = 0
    ...
    if not final_test_loader.dataset.X_data.size(0) == 0:
        ...
        ##############################################
        # 8) Zusätzlicher Plot für 200 Stunden ab 01.04.2023/2024
        ##############################################
        ...
        # Finde den Index, der dem Startdatum entspricht
        ...
        if len(start_idx) == 0:
            ...
            # Stelle sicher, dass der Endindex nicht die Länge überschreitet
            ...
            # Extrahiere die relevanten Vorhersagen und Zielwerte
            preds_april = preds_rescaled[start_idx:end_idx]
            targets_april = targets_rescaled[start_idx:end_idx]

            # Optional: Erhalte die zugehörigen Zeitstempel für die x-Achse
