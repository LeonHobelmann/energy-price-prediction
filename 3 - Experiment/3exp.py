...
def train_and_validate(
    ...
    feature_importances = np.mean(np.abs(weight_ih), axis=0)

    feature_importances_normalized = feature_importances / np.max(feature_importances)

    top_10_indices = np.argsort(feature_importances_normalized)[-10:][::-1]  # Indizes der Top 10
    top_10_features = [feature_names[i] for i in top_10_indices]
    top_10_importances = feature_importances_normalized[top_10_indices]

    plt.figure(figsize=(12, 6))
    plt.bar(range(len(top_10_importances)), top_10_importances, tick_label=top_10_features, color='skyblue')
    plt.xticks(rotation=45, ha='right')
    plt.xlabel("Top 10 Features")
    plt.ylabel("Normalisierte durchschnittliche absolute Gewichtung")
    plt.title("Top 10 Feature-Gewichtungen im LSTM-Modell")
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    file_path = "11Daten 2020 - 2023.xlsx"
    ...
    def split_Xy(df_in):
        ...
        feature_names = df_in.drop(columns=["Intraday"]).columns.tolist()
        return X_, y_, feature_names


    X_train_raw, y_train_raw, feature_names = split_Xy(df_train)
    X_val_raw, y_val_raw, _ = split_Xy(df_val)
    X_test_raw, y_test_raw, _ = split_Xy(df_test)
    ...
    model, test_preds, test_targets = train_and_validate(
        ...
        X_test_scaled, y_test_scaled,
        feature_names,
        EPOCHS=50, LEARNING_RATE=0.000, PATIENCE=5, BATCH_SIZE=1024
    )
